...
def setup_args():
    ...
    train.add_argument("-is_test","--is_test",type=bool,default=False)
    train.add_argument("-embedding_type","--embedding_type",type=str,default='random')
    train.add_argument("-epoch","--epoch",type=int,default=100)
...
class TrainLoop_fusion_rec():
    def __init__(self, opt, is_finetune):
        ...
        self.metrics_rec={"recall@1":0,"recall@10":0,"recall@50":0,"loss":0,"count":0,'mrr@1':0, 'mrr@10':0, 'mrr@50':0}
    ...
    def train(self):
        ...
        for i in range(3):
            ...
            # #print('valid_dataset val:')
            # #output_metrics_rec = self.val()
            # #print('test_dataset val:')
        ...
        self.model.save_pretrained_model()
        ...
        for i in range(self.epoch):
            ...
            for context,c_lengths,response,r_length,mask_response,mask_r_length,entity,entity_vector,movie,concept_mask,dbpedia_mask,concept_vec, db_vec,rec in tqdm(train_dataset_loader):
                ...
                scores, preds, rec_scores, rec_loss, gen_loss, mask_loss, info_db_loss, _=self.model(context.cuda(), response.cuda(), mask_response.cuda(), concept_mask, dbpedia_mask, seed_sets, movie,concept_vec, db_vec, entity_vector.cuda(), rec, test=False, epoch = i)
    ...
    def metrics_cal_rec(self,rec_loss,scores,labels):
        ...
        for b in range(batch_size):
            ...
            if target_idx in pred_idx[b][:1].tolist():
                self.metrics_rec['mrr@1'] += 1 / (pred_idx[b][:1].tolist().index(target_idx) + 1)
            if target_idx in pred_idx[b][:10].tolist():
                self.metrics_rec['mrr@10'] += 1 / (pred_idx[b][:10].tolist().index(target_idx) + 1)
            if target_idx in pred_idx[b][:50].tolist():
                self.metrics_rec['mrr@50'] += 1 / (pred_idx[b][:50].tolist().index(target_idx) + 1)

    def val(self,is_test=False):
        self.metrics_gen={"ppl":0,"dist1":0,"dist2":0,"dist3":0,"dist4":0,"bleu1":0,"bleu2":0,"bleu3":0,"bleu4":0,"count":0}
        self.metrics_rec={"recall@1":0,"recall@10":0,"recall@50":0,"loss":0,"gate":0,"count":0,'gate_count':0, 'mrr@1':0, 'mrr@10':0, 'mrr@50':0}
        self.model.eval()
        if is_test:
...
class TrainLoop_fusion_gen():
    ...
    def metrics_cal_gen(self,rec_loss,preds,responses,recs):
        ...
        def distinct_metrics(outs):
            ...
            dis1 = len(unigram_set) /len(outs)    #unigram_count#len(outs)
...
if __name__ == '__main__':
    ...
    if args.is_test==True:
        ...
        if args.is_finetune==False:
            print('is_finetune=False')
            loop=TrainLoop_fusion_rec(vars(args),is_finetune=False)
            #loop.model.load_model()
            loop.train()
        else:
            print('is_finetune=True')
